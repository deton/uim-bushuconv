...
1. `<Control>r`を1回押すと"七"を部首として取得。
2. 2回目に押すと"六"を部首として取得(部首リストは"七六")。
3. 3回目に押すと"五"を部首として取得
   (ただし、"七六五"で合成できる漢字が無いため"五"は削除されて、
   部首リストは"七六"のまま)。
4. 4回目に押すと"四"を部首として取得(部首リストは"七六四")。

`<Control>s`を使う例:

1. `<Control>r`を1回押すと"七"を部首として取得。
2. `<Control>s`を押すと、再度"七"を部首として取得。
...
* クリップボードから部首のリストをペーストした際や、
  ...
  (期待する動作は、その部首だけスキップ)。
    ...
    `<Control>r`による部首取得を使えば、
    途中に無視される部首があっても次の部首を取得することが可能です。
