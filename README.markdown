...
* 複数の部首を含むセレクション文字列を使った部首合成変換可能
    ...
    画数を数えて部首を選ぶのが面倒な場合、
    uim-bushuconvに切替後に「角傷」の2文字をペーストするか、
    エディタに「角傷」を入力・選択後にuim-bushuconvに切替。

    なお、uim-tutcodeの対話的な部首合成変換機能では、
...
なお、クリップボードからの文字列取得やセレクション文字列の取得は、
...
例えば、漢字集合"JIS X 0208"の場合、
「りっしんべん」を代表する文字として「性」が使われています。

また、漢字集合"JIS X 0208"用の部首合成変換辞書は、
もともと部首2文字で漢字を合成する目的で作成されたものなので、
漢字を構成する全ての部品が記述されていない場合があります。
そのため、部品を入力していっても、目的の漢字が候補に出てこない場合があります。
(例:bushu12.revでは、「操」は「才品」から合成することになっているため、
"才木"を入力しても「操」は候補に出てこない)
...
* left    選択中の漢字候補のUnicodeコードポイント"U+xxxx"を確定する

カスタマイズ項目
================

* 漢字集合 (デフォルト: "JIS X 0213")
    + "JIS X 0208": JIS第1・第2水準
    + "JIS X 0213": JIS第3・第4水準(JIS X 0208を含む)
    ...
    EUC-JP(EUC-JIS-2004)環境では、
    補助漢字(JIS X 0212)を確定しても使用できない可能性が高いです。
    (確定しようとしてる漢字が補助漢字かどうかは、註釈表示で確認可能です)
    ...
    Adobe-Japan1-6対応フォント(OpenTypeフォントPr6/Pr6N)なら
...
* [漢字検索](http://x0213.org/searchkanji/)
    ...
    部首名(例:「のぎへん」)、読み、総画数等を組み合わせて漢字の検索が可能。
